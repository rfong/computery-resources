...
- If you feel the pain most in your fingertips or nerves at the moment you strike the keys, then you could probably benefit from lighter switches. But if you are overstriking, then lighter switches alone won't magically solve problems; you might also need to retrain yourself to type more lightly. Tactile switches can help with this. If you feel fingertip pain even when pressing on a key very slowly and gently, then your problems might indeed be addressed just by lighter-weight switches.
...
- If moving your fingers off homerow hurts, try a layered layout on a smaller board so that you don't need to move your hands much.
...
**Body context:** I am a vim user and touch typist with very minor thumb arthritis (probably from smartphones). No wrist issues yet; probably because I have classical piano training. In 2021, I developed ulnar nerve damage from a computer-unrelated incident which made it excruciatingly painful to type with my pinkies. Prior to that, my work keyboard was a WASD mechanical keyboard with standard tactile switches and a standard layout. QWERTY was not the source of my pain, so I have stayed on it. I don't like tucking my thumbs underneath my hands; there are some physical layouts you can try if you prefer that. I span a ninth chord on the piano. I have a strong visual memory and I wanted an extremely portable board, so I went with a layered 36-key approach.

**[My custom virtual layout](https://github.com/rfong/shibui/blob/main/KEYMAP.md)** requires 36 physical keys, 2 of which are used for layer switching. This results in 4 layers that give me 124 virtual key slots, not counting mod-tap switching. The 3rd layer is mostly for shortcuts/macros and the 4th layer is mostly for hardware functions such as bluetooth connection switching; in practice. I only use around 100 of my available key slots.

My first split ergomech keyboard in 2021 was a wired **[crkbd/Corne](https://github.com/foostan/crkbd)** (open-source split 40% ortholinear keyboard), which makes great use of layers and has an exceptionally comfortable thumb layout that lets you transfer a lot of traditional pinky work to the thumbs. I also hand-built a custom [trackpoint hotswap mod](https://github.com/manna-harbour/crkbd/issues/1#issuecomment-953258378) so that I don't have to use a mouse. I got RGB LED underlighting to look cool. In practice, I don't really use the LEDs.
...
On my low-profile Chocofi, I use 20g low-profile linear "Light Blue" switches with LDSA spherical keycaps. I tried the 35g tactile "Sunset" switches, which are the lightest low-profile tactiles you can get, but they felt a lot heavier than 35g regular-height switches would. This is a known phenomenon where the super-short switch throw feels heavier than its rating. You also hit the bottom of the switch faster, so there's not as big of a difference between how linears and tactiles feel.

Keycaps actually make a big difference on low-profile switches because the switch stem is so short. In my experience, if I strike the flat-profile MBK keycap off to the side, it cantilevers the short switch, adding a lot of friction and making the switch feel heavier. The LDSA spherical keycap only allows me to type straight down into the center of the stem, minimizing friction.
